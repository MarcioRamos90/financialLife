...
// Wrap renders in a QueryClientProvider so that any sub-component that calls
// useQuery (e.g. TransactionForm's usePaymentMethods) has a client available,
// even when vi.spyOn does not fully intercept cross-module import bindings.
...
describe('TransactionList', () => {
  ...
  it('shows the three summary cards', () => {
    mockHooks()
    renderWithClient(<TransactionList />)
    // Use { selector: 'p' } to avoid matching identically-named <option> elements
    // that also appear in the type filter <select>.
  ...
  it('calculates the surplus correctly from mixed transactions', () => {
    ...
    mockHooks({ transactions: txs })
    renderWithClient(<TransactionList />)
    // Surplus = 5000 - 1200 = 3800; formatted as Brazilian Real
  ...
  it('opens the new-transaction modal when + New transaction is clicked', () => {
    mockHooks()
    renderWithClient(<TransactionList />)
    fireEvent.click(screen.getByRole('button', { name: /new transaction/i }))
    // TransactionForm renders its own hooks; mock them to avoid errors.
    // We just check that the modal header appears.
  ...
  it('calls deleteTransaction with the correct id on confirmation', async () => {
    ...
    const confirmBtn = screen.getAllByRole('button', { name: /^delete$/i })
      .find(b => b.closest('.fixed')) // the one inside the modal overlay
